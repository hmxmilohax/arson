...
pub fn register_funcs(context: &mut Context) {
    context.add_required_macro("kDataInt", arson_array![NodeKind::Integer]);
    context.add_required_macro("kDataFloat", arson_array![NodeKind::Float]);
    context.add_required_macro("kDataString", arson_array![NodeKind::String]);
    context.add_required_macro("kDataSymbol", arson_array![NodeKind::Symbol]);
    context.add_required_macro("kDataVar", arson_array![NodeKind::Variable]);

    context.add_required_macro("kDataFunc", arson_array![NodeKind::Function]);
    context.add_required_macro("kDataObject", arson_array![NodeKind::Object]);

    context.add_required_macro("kDataArray", arson_array![NodeKind::Array]);
    context.add_required_macro("kDataCommand", arson_array![NodeKind::Command]);
    context.add_required_macro("kDataProperty", arson_array![NodeKind::Property]);

    // Already handled by the parser, but may as well for completeness
    context.add_required_macro("kDataUnhandled", arson_array![NodeKind::Unhandled]);
